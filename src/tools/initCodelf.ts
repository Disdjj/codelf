...
export const initCodelf = (server: any) => {
    server.tool(
        ...
        1. When handling rootPath and analyzeDirectorys, if you encounter Percent-encoding (such as "%3A"), please decode it to the original Windows path format (e.g., "D:\code\codelf") to avoid path errors.
        2. If User provide the analyzeDirectorys, you need to convert to an absolute path to ensure correct function's invocation and push to an Array as an "analyzeDirectorys" before you read and update the yaml fileds.
        3. Tell users the conversion results and the analyzeDirectorys.
    ...
    `,
        {
           ...
           "C:/User/name/codeProject" or "/src" in windows
           "/usr/name/codeProject/" or "/src" in macos/linux`
            ),
        },
        ...
        3. And then, if you want to update the YAML and user has provided the analyzeDirectorys, just need to compare the directory structure and YAML in the actual project base on the ${analyzeDirectorys}
        ...
        `,
                ...
                await fs.writeFile(projectinfoYaml, "structure:\n", "utf-8");
